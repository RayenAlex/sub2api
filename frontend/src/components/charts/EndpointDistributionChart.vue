<template>
  <div
    ...
    :class="telemetry ? 'telemetry-distribution-card telemetry-distribution-card--amber' : 'card p-4'"
  >
    ...
    <div v-else-if="displayEndpointStats.length > 0 && chartData" class="telemetry-distribution-body flex flex-col items-center gap-4 sm:flex-row sm:gap-6">
      <div :class="telemetry ? 'telemetry-donut-shell' : 'h-48 w-48 shrink-0'">
        ...
        <div v-if="telemetry" class="telemetry-donut-center" aria-hidden="true">
          ...
          <small data-testid="telemetry-donut-active">{{ displayEndpointStats.length }} 个活跃端点</small>
        </div>
      </div>
      <div class="max-h-48 w-full min-w-0 flex-1 overflow-auto">
        <table class="w-full text-xs">
          ...
          <tbody>
            <template v-for="(item, endpointIndex) in displayEndpointStats" :key="item.endpoint">
              <tr
              ...
              >
                <td class="max-w-[180px] truncate py-1.5 font-medium" :class="enableBreakdown ? 'text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300' : 'text-gray-900 dark:text-white'" :title="item.endpoint">
                  <span class="inline-flex items-center gap-1">
                    ...
                    <span v-if="telemetry" class="telemetry-series-swatch" :style="{ backgroundColor: chartColors[endpointIndex % chartColors.length] }"></span>
                    {{ item.endpoint }}
                  </span>
                </td>
    ...
    <div v-if="telemetry" data-testid="telemetry-distribution-footer" class="telemetry-distribution-footer">
      <span>筛选条件：{{ source === 'inbound' ? '全部入站端点' : source === 'upstream' ? '全部上游端点' : '全部请求路径' }}</span>
...
const telemetryAggregate = computed(() => {
  const total = displayEndpointStats.value.reduce(
    (sum, item) => sum + Number(props.metric === 'actual_cost' ? item.actual_cost || 0 : item.total_tokens || 0),
    0
  )
