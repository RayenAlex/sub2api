...
func TestAPIKeyService_RejectsV25AuthSnapshotWithoutTokenQuotaLimits(t *testing.T) {
	svc := &APIKeyService{}

	apiKey, ok, err := svc.applyAuthCacheEntry("k-legacy-token-quota", &APIKeyAuthCacheEntry{
		Snapshot: &APIKeyAuthSnapshot{Version: 25},
	})
	require.NoError(t, err)
	require.False(t, ok, "v25 snapshots were created before auth projection included token quota limits")
