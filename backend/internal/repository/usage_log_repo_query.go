...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	var (
		...
		appliedPeakMultiplier     sql.NullFloat64
		...
		cacheDiagnostic           sql.NullString
	...
	log := &service.UsageLog{
		...
		AppliedPeakMultiplier:     nullFloat64Ptr(appliedPeakMultiplier),
	...
	if cacheDiagnostic.Valid {
		diagnostic := &service.OpenCodeCacheDiagnostic{}
		if err := json.Unmarshal([]byte(cacheDiagnostic.String), diagnostic); err == nil {
			log.CacheDiagnostic = diagnostic
		}
	}
	if sessionID.Valid {
