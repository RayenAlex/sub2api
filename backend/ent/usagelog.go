...
type UsageLog struct {
	...
	// AppliedPeakMultiplier holds the value of the "applied_peak_multiplier" field.
...
func (*UsageLog) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case usagelog.FieldInputCost, usagelog.FieldOutputCost, usagelog.FieldCacheCreationCost, usagelog.FieldCacheReadCost, usagelog.FieldTotalCost, usagelog.FieldActualCost, usagelog.FieldRateMultiplier, usagelog.FieldAppliedPeakMultiplier, usagelog.FieldAccountRateMultiplier:
...
func (_m *UsageLog) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case usagelog.FieldAppliedPeakMultiplier:
			if value, ok := values[i].(*sql.NullFloat64); !ok {
				return fmt.Errorf("unexpected type %T for field applied_peak_multiplier", values[i])
			} else if value.Valid {
				_m.AppliedPeakMultiplier = new(float64)
				*_m.AppliedPeakMultiplier = value.Float64
			}
...
func (_m *UsageLog) String() string {
	...
	builder.WriteString(", ")
	if v := _m.AppliedPeakMultiplier; v != nil {
		builder.WriteString("applied_peak_multiplier=")
		builder.WriteString(fmt.Sprintf("%v", *v))
	}
	builder.WriteString(", ")
